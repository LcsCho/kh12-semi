...
public class ReservationDaoImpl implements ReservationDao {
	...
	public void insert(ReservationDto reservationDto) {
		...
		Object[] data = { reservationDto.getMatchNo(), reservationDto.getSeatNo(), reservationDto.getHomeTeam(),
				reservationDto.getAwayTeam(), reservationDto.getMemberId(), reservationDto.getReservationDate(),
				reservationDto.getStadiumName(), reservationDto.getReservationDate(), reservationDto.getSeatAreaPrice(),
				reservationDto.getStadiumNo(), reservationDto.getSeatAreaZone(), reservationDto.getSeatRow(),
	...
	public List<ReservationDto> selectList(String memberId) {
		...
		String sql = "SELECT rs.reservation_no, " + "ma.match_no, " + "s.seat_no, " + "hm.TEAM_name as HOME_TEAM, "
				+ "aw.team_name as AWAY_TEAM, " + "m.member_id, " + "ma.stadium_name, " + "rs.RESERVATION_DATE, "
				+ "s.seat_area_no, " + "sa.seat_area_price, " + "sa.stadium_no, " + "sa.seat_area_zone, "
				+ "s.seat_row, " + "s.seat_col," + "ma.match_date, " + "rs.reservation_ticket " + "FROM reservation rs "
				+ "INNER JOIN seat s ON rs.seat_no = s.seat_no " + "INNER JOIN match ma ON rs.match_no = ma.match_no "
				...
				+ "INNER JOIN seat_area sa ON rs.seat_area_no = sa.SEAT_AREA_NO " + "WHERE m.member_id = ?";
	...
	public List<ReservationVO> getMatchInfo(int matchNo) {
		String sql = "SELECT rs.reservation_no, " + "ma.match_no, " + "s.seat_no, " + "hm.TEAM_NO as HOME_TEAM, "
				+ "aw.team_no as AWAY_TEAM, " + "m.member_id, " + "ma.stadium_name, " + "rs.RESERVATION_DATE, "
				+ "s.seat_area_no, " + "sa.seat_area_price, " + "sa.stadium_no, " + "sa.seat_area_zone, "
				+ "s.seat_row, " + "s.seat_col, " + "ma.MATCH_DATE, " + "rs.RESERVATION_TICKET "
				+ "FROM reservation rs " + "INNER JOIN seat s ON rs.seat_no = s.seat_no "
				+ "INNER JOIN match ma ON rs.match_no = ma.match_no "
				+ "INNER JOIN member m ON rs.member_id = m.member_id "
				+ "INNER JOIN team hm ON rs.home_team = hm.team_no "
				+ "INNER JOIN team aw ON rs.away_team = aw.team_no "
				+ "INNER JOIN seat_area sa ON rs.seat_area_no = sa.SEAT_AREA_NO " + "WHERE ma.MATCH_NO = ?";
	...
	public List<AdminReservationListVO> reservationListByAdmin(PaginationVO vo) {
		String sql = "select * from (" + "select rownum rn, TMP.* from (" + "SELECT ma.HOME_TEAM," + " ma.AWAY_TEAM,"
				+ " sa.seat_area_zone," + " s.SEAT_ROW," + " s.SEAT_COL," + " rs.member_id," + " rs.RESERVATION_DATE,"
				+ " rs.RESERVATION_NO," + " ma.STADIUM_NAME," + " sa.SEAT_AREA_PRICE," + " s.seat_no" + " FROM"
				+ " reservation rs " + "INNER JOIN match ma ON rs.match_no = ma.MATCH_NO "
				+ "INNER JOIN seat_area sa ON rs.SEAT_AREA_NO = sa.SEAT_AREA_NO "
				+ "INNER JOIN seat s ON s.SEAT_NO = rs.SEAT_NO" + ")TMP" + ") where rn between ? and ?";
...
// 죄석 상태를 select 로 parameter 로 받아서 넘김
	@Override
	public boolean seatBlock(SeatUpdateVo seatUpdateVo) {
	    String sql = "update seat set seat_status = ? where seat_no=?";
	    
	    int seatNos[] = seatUpdateVo.getSeatNo();
	    int count = seatNos.length;
	    
	    for (int i = 0; i < count; i++) {
	        Object[] data = { seatUpdateVo.getSeatStatus(), seatNos[i] };
	        // 업데이트 쿼리 실행
	        int updatedRows = jdbcTemplate.update(sql, data);
	        
	        // 업데이트 작업이 실패하면 false 반환
	        if (updatedRows <= 0) {
	            return false;
	        }
	    }
	    
	    // 모든 업데이트 작업이 성공하면 true 반환
	    return true;
	}
	...
	@Override
	public List<SeatListDto> selectListForUpdate(String seatAreaZone, String stadiumName) {
		String sql = "SELECT s.seat_no, sa.seat_area_no, st.stadium_name, sa.seat_area_price, sa.seat_area_zone, "
				+ "s.seat_col, s.seat_row, s.seat_status, st.stadium_no " + "FROM seat s "
		...
		Object[] data = { seatAreaZone, stadiumName  };
